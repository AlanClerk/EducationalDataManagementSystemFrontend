  ...
  try {
    ...
    if (res.data.code === 200) {
      grades.value = res.data.rows.map(grade => {
        const course = courses.value.find(c => c.courseId === String(grade.courseId))
        return {
          ...grade,
          courseId: String(grade.courseId), // 统一为字符串
          courseName: course ? course.courseName : grade.courseId
        }
      })
...
td button:first-child {
  background-color: #67c23a; /* 编辑按钮为绿色 */
}

td button:first-child:hover:not(:disabled) {
  background-color: #85ce61; /* 编辑按钮悬停颜色 */
}

td button:last-child {
  background-color: #52a01e; /* 删除按钮为深绿色 */
}

td button:last-child:hover:not(:disabled) {
  background-color: #6bb83a; /* 删除按钮悬停颜色 */
}
